...
from eiffel_loop.os.util import *
...
class INSTALLER: # Common: Unix and Windows
	...
	def build_eiffel_tool (self):
		...
		if not os_env.command_exists (['el_eiffel', '-version', '-no_highlighting'], shell=self.is_windows ()):
...
class WINDOWS_INSTALLER (INSTALLER):
	...
	def install_scons (self):
		if not os_env.command_exists (['scons', '-v'], shell = True):
	...
	def install_lxml (self):
		try:
			...
			s = input ('Press <return> to download and install')
			pkg_lxml.download ()
	...
	def install_gedit_pecf_support (self):
		...
		for extension_name, pyxis_key_name, description, icon_path, extension_cmds in mime_types:
			...
			for command_name, command in extension_cmds.items():
...
class POSIX_INSTALLER (INSTALLER):
	...
	def install (self):
		...
		os.chmod (launch_estudio_path, 0o777)
	...
	def install_gedit_pecf_support (self):
		...
		print('Calling:', update_cmd, end=' ')
		if int (subprocess.call (update_cmd)) == 0:
			print('OK')
		else:
			print('FAILED')

installer = os_imp (INSTALLER)
