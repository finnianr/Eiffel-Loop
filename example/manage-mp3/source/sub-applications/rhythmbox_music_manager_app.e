...
note
	...
	instructions: "[
		...
		**INSTALLATION**
		...
		Strip off the version number by renaming the file and then copy it into `/usr/local/bin' directory.
		...
		**TASK LIST**
		...
		Recognised extensions are: flv, m4a, m4v, mp4, mov. Includes facility for mapping video segments to individual
		MP3 files.
		...
		This task is intended for use by tango DJs to create
		[https://en.wikipedia.org/wiki/Cortina_(tango) tango cortinas] to act as breaks between 
		tandas (a set of dance songs).
		
		You will be prompted to drag and drop a song to use for the cortina. The entire song will be divided up into fixed
		length cortinas with a fade-in and fade-out. The cortina titles will be named so they can be use to indicate the
		genre of the tanda following. The frequency of genre titles assumes you wish to construct tandas using the
		Buenos Aires format of genre alternation. A leading letter serves to make all titles unique.
		...
		A typical DJ event playlist will look like this when you have finished editing it.
			...
			 pyxis-doc:
				version = 1.0; encoding = "UTF-8"
			DJ-event:
				title = "Compadrito Milonga Playlist"; date = "10/6/2010"; start_time = "21:16"
				venue = "The Globe Pub"; DJ_name = "Finnian Reilly"; ignore = false
				playlist:
					path:
