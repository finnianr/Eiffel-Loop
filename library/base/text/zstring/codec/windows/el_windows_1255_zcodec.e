	...
	latin_character (uc: CHARACTER_32): CHARACTER
		...
		do
			inspect uc
				...
				when 'ְ'..'ֹ' then
					Result := latin_set_2 [uc.code - 1456]
				when 'ֻ'..'׃' then
					Result := latin_set_3 [uc.code - 1467]
				when 'װ'..'״' then
					Result := latin_set_4 [uc.code - 1520]
				when '‘'..'‚' then
				...
				when '‎'..'‏' then
					Result := latin_set_10 [uc.code - 8206]
				when '×' then
					Result := '%/170/'
				when '₪' then
					Result := '%/164/'
...
feature -- Character query
	...
	in_latin_1_disjoint_set (c: CHARACTER): BOOLEAN
		...
		do
			inspect c
				when Substitute, '%/0128/', '%/0130/'..'%/0137/', '%/0139/', '%/0145/'..'%/0153/', '%/0155/', '¤', 'ª', 'º', 'À'..'É', 'Ë'..'Ø', 'à'..'ú', 'ý' then
					Result := True
			else
			end
		end
