note
	...
	date: "2019-09-28 19:13:34 GMT (Saturday   28th   September   2019)"
	revision: "5"
	...
	latin_character (uc: CHARACTER_32; unicode: INTEGER): CHARACTER
		...
		do
			inspect uc
				...
				when '–'..'—' then
					Result := latin_set_4 [unicode - 8211]
				when 'Ž'..'ž' then
					Result := latin_set_5 [unicode - 381]
				when '‹'..'›' then
					Result := latin_set_6 [unicode - 8249]
				when 'Š'..'š' then
					Result := latin_set_7 [unicode - 352]
				when 'Œ'..'œ' then
					Result := latin_set_8 [unicode - 338]
				when '€' then
					Result := '%/128/'
				when '™' then
					Result := '%/153/'
