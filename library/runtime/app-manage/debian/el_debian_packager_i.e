note
	description: "[
		...
		that inherits ${EL_MULTI_APPLICATION_ROOT}.
	]"
...
note
	notes: "[
		**Platform Implementation**
		...
		As the application may also need to compile on Windows, class ${EL_DEBIAN_PACKAGER_IMP} isolates
		functions that are specific to Unix, specifically the creation of XDG desktop entries. See routine `put_xdg_entries'.

		**Sub Application**

		By including ${EL_DEBIAN_PACKAGER_APP} in the list of sub-application types listed in the root
		class implementing ${EL_MULTI_APPLICATION_ROOT}, the application becomes capable of generating
		it's own install package. At least one sub-application must conform to ${EL_INSTALLABLE_APPLICATION}.
	...
	instructions: "[
		...
		**Output**
		...
		By default running the sub-application ${EL_DEBIAN_PACKAGER_APP} will generate the package
