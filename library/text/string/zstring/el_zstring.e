note
	description: "Summary description for {EL_ASTRING_2}."
	...
	date: "2016-01-21 14:31:46 GMT (Thursday 21st January 2016)"
	revision: "5"
...
convert
	make_from_unicode ({STRING_32}), make_from_latin_1 ({STRING}),
...
feature -- Element change
	...
	append_substring_general (s: READABLE_STRING_GENERAL; start_index, end_index: INTEGER)
		do
			if attached {EL_ZSTRING} s as str_z then
				append_substring (str_z, start_index, end_index)
			else
				append_substring (adapted_general (s), start_index, end_index)
			end
		end
	...
	insert_string_general (s: READABLE_STRING_GENERAL; i: INTEGER)
		do
			insert_string (adapted_general (s), i)
		end
	...
	prepend_string_general (str: READABLE_STRING_GENERAL)
		do
			if attached {EL_ZSTRING} str as str_z then
				prepend_string (str_z)
			else
				prepend_string (adapted_general (str))
			end
		ensure then
	...
	replace_delimited_substring_general (
		...
		local
			l_left, l_right: ZSTRING
		do
			l_left := adapted_general (left).twin; l_right := adapted_general (right).twin
			replace_delimited_substring (l_left, l_right, adapted_general (new), include_delimiter, start_index)
		end
	...
	replace_substring_general (s: READABLE_STRING_GENERAL; start_index, end_index: INTEGER)
		do
			if attached {EL_ZSTRING} s as str_z then
				replace_substring (str_z, start_index, end_index)
			else
				replace_substring (adapted_general (s), start_index, end_index)
			end
		end
	...
	replace_substring_general_all (original, new: READABLE_STRING_GENERAL)
		...
		do
			arguments := adapted_arguments (original, new)
			replace_substring_all (arguments [1], arguments [2])
		end
...
feature {NONE} -- Implementation
	...
	adapted_general (a_unicode: READABLE_STRING_GENERAL): like Current
		do
			...
			Result.append_string_general (a_unicode)
		end
