...
feature -- Access
	...
	substring_index_general (other: READABLE_STRING_GENERAL; start_index: INTEGER): INTEGER
		do
			if attached {EL_READABLE_ZSTRING} other as other_z then
				Result := substring_index (other_z, start_index)
			else
				Result := substring_index (adapted_general (other), start_index)
			end
		end
	...
	substring_index_in_bounds_general (other: READABLE_STRING_GENERAL; start_pos, end_pos: INTEGER): INTEGER
		do
			Result := substring_index_in_bounds (adapted_general (other), start_pos, end_pos)
		end
	...
	word_index_general (word: READABLE_STRING_GENERAL; start_index: INTEGER): INTEGER
		do
			Result := word_index (adapted_general (word), start_index)
		end
...
feature -- Status query
	...
	enclosed_with_general (character_pair: READABLE_STRING_GENERAL): BOOLEAN
		require
			is_pair: character_pair.count = 2
		do
			if attached {EL_READABLE_ZSTRING} character_pair as character_pair_z then
				Result := enclosed_with (character_pair_z)
			else
				Result := enclosed_with (adapted_general (character_pair))
			end
		end
	...
	ends_with_general (str: READABLE_STRING_GENERAL): BOOLEAN
		do
			if attached {EL_READABLE_ZSTRING} str as str_z then
				Result := ends_with (str_z)
			else
	...
	starts_with_general (str: READABLE_STRING_GENERAL): BOOLEAN
		do
			Result := starts_with (adapted_general (str))
		end
...
feature {EL_READABLE_ZSTRING} -- Element change
	...
	translate_general (old_characters, new_characters: READABLE_STRING_GENERAL)
		local
			arguments: like adapted_arguments
		do
			arguments := adapted_arguments (old_characters, new_characters)
			translate (arguments [1], arguments [2])
		end
...
feature {NONE} -- Implementation
	...
	adapted_general (a_unicode: READABLE_STRING_GENERAL): like Current
		require
			not_zstring: not attached {like Current} a_unicode
		deferred
		end

	adapted_arguments (a_arg_1, a_arg_2: READABLE_STRING_GENERAL): ARRAY [like Current]
		local
			arg_1, arg_2: EL_READABLE_ZSTRING; arg_1_adapted: BOOLEAN
		do
			if attached {EL_READABLE_ZSTRING} a_arg_1 as str_z then
				arg_1 := str_z
			else
				arg_1 := adapted_general (a_arg_1)
				arg_1_adapted := True
			end
			if attached {EL_READABLE_ZSTRING} a_arg_2 as str_z then
				arg_2 := str_z
			else
				if arg_1_adapted then
					arg_1 := arg_1.twin
				end
				arg_2 := adapted_general (a_arg_2)
			end
			Result := << arg_1, arg_2 >>
		end
