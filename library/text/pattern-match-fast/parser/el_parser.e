note
	...
	date: "2022-11-01 15:21:00 GMT (Tuesday 1st November 2022)"
	revision: "11"
...
feature {NONE} -- Initialization
	...
	make_default
			--
		do
--			source_text := Default_source_view
...
feature -- Element change
	...
	set_substring_source_text (a_source_text: READABLE_STRING_GENERAL; start_index, end_index: INTEGER)
		local
			s_8: EL_STRING_8_ROUTINES; s_32: EL_STRING_32_ROUTINES
			z: EL_ZSTRING_ROUTINES
		do
			if attached {STRING_8} source_text as str_8 then
				set_source_text (s_8.shared_substring (str_8, end_index))
				start_offset := start_index - 1

			elseif attached {STRING_32} source_text as str_32 then
				set_source_text (s_32.shared_substring (str_32, end_index))
				start_offset := start_index - 1

			elseif attached {ZSTRING} source_text as z_str then
				set_source_text (z.shared_substring (z_str, end_index))
				start_offset := start_index - 1

			elseif attached {IMMUTABLE_STRING_8} source_text as str_8 then
				set_source_text (str_8.shared_substring (start_index, end_index))

			elseif attached {IMMUTABLE_STRING_32} source_text as str_32 then
	...
	match_full
		...
		do
			if attached pattern as l_pattern then
				...
				fully_matched := l_pattern.count = source_text.count
			end
		end
