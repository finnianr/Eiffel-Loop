note
	...
	date: "2018-09-27 13:25:48 GMT (Thursday 27th September 2018)"
	revision: "5"
...
feature {NONE} -- Initialization
	...
	make (a_output_dir: EL_DIR_PATH; a_character_set: EL_ENCODING)
			--
		do
			Precursor (a_output_dir, a_character_set)
			create h2_list.make
		end
...
feature {NONE} -- State handlers
	...
	find_end_image_paragraph (line: ZSTRING)
		do
			if begins (line, Paragraph_tag.closed) then
	...
	find_end_tag (line: ZSTRING)
		...
		do
			...
			elseif begins (line, Preformat_tag) then
				extend_html (line)
	...
	find_right_angle_bracket (line: ZSTRING)
		do
			if ends_with_character (line, '>') then
				state := agent find_end_tag
			end
		end
	...
	find_start_image_paragraph (line: ZSTRING)
		do
			if begins (line, Paragraph_tag.open) then
...
feature {NONE} -- Implementation
	...
	extend_html (line: ZSTRING)
		...
		do
			if begins (line, H2_tag.open) then
