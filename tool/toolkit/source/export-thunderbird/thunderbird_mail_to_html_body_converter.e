...
feature -- Basic operations
	...
	convert_mails (mails_path: EL_FILE_PATH)
		local
			order_file: EL_PLAIN_TEXT_FILE; order_lines: EL_ASTRING_LIST
...
feature {NONE} -- State handlers
	...
	find_end_tag (line: ASTRING)
		local
			pos_class: INTEGER; extra_lines: EL_ASTRING_LIST
		do
			...
			elseif line.starts_with (once "<pre>") then
				extend_html (line)
	...
	find_start_image_paragraph (line: ASTRING)
		do
	...
	find_end_image_paragraph (line: ASTRING)
		do
	...
	find_preformatted_end_tag (line: ASTRING)
		do
	...
	find_right_angle_bracket (line: ASTRING)
		do
...
feature {NONE} -- Implementation
	...
	append_to_html (lines: ARRAY [ASTRING])
		do
	...
	extend_html (line: ASTRING)
		local
			heading_text: ASTRING; pos_right_bracket: INTEGER
		do
...
feature {NONE} -- Constants
	...
	Paragraph_tag: TUPLE [open, closed: ASTRING]
		once
	...
	First_h2_class_attribute: ASTRING
		once
			Result := " class=%"first%""
		end
	...
	Header_list_extension: ASTRING
		once
			Result := "h2"
		end

	Image_open_tag: ASTRING
		once
			Result := "<img"
		end

	Class_paragraph_image: ASTRING
		once
