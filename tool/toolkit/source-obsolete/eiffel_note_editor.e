...
feature -- Element change
	...
	set_source_text_from_line_source (lines: EL_FILE_LINE_SOURCE)
		...
		do
			...
			time_stamp := date_field_stamp.relative_duration (Date.epoch).seconds_count.to_integer_32
...
feature {NONE} -- Parsing actions
	...
	on_field_text (text: EL_STRING_VIEW)
		...
		do
			...
			elseif Field_names.has (last_field_name) then
	...
	on_verbatim_field_text (text: EL_STRING_VIEW)
		...
		do
			...
			if Field_names.has (last_field_name) then
...
feature -- Constants
	...
	Class_declaration: EL_MATCH_ALL_IN_LIST_TP
		...
		once
			Result := all_of (<<
				optional (all_of (<< string_literal (Keyword_deferred), white_space >>)),
				whole_word (Keyword_class)
			>>)
		end
