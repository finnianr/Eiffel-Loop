note
	description: "[
		Tests for classes ASTRING and EL_FILE_LINE_SOURCE
	]"
	...
	test_append_string
		...
		do
			...
			across Russian_proverb_words as word loop
				...
				a.append (create {ASTRING}.make_from_unicode (word.item))
...
--			change_case (Lower_case_mu, Upper_case_mu)
	...
	test_index_of
		note
			testing: "covers/{ASTRING}.index_of"
		local
			characters, uc_proverb: STRING_32; proverb: ASTRING
	...
	test_insert_string
		note
			testing:	"covers/{ASTRING}.insert_string"
		local
			uc_incomplete_proverb: STRING_32
			incomplete_proverb, word: ASTRING
			pos_word: INTEGER
		do
	...
	test_left_right_adjustments
		note
			testing:	"covers/{ASTRING}.left_adjust", "covers/{ASTRING}.right_adjust"
		local
			str: ASTRING; str_32, white_space, white_space_32: STRING_32
		do
			across Russian_proverb_words as word loop
	...
	test_make_from_unicode
		note
			testing:	"covers/{ASTRING}.make_from_unicode", "covers/{ASTRING}.to_unicode"
		local
			str: ASTRING
		do
	...
	test_prepend
		local
			str_32: STRING_32; str, word: ASTRING
	...
	test_put_unicode
		note
			testing:	"covers/{ASTRING}.make_from_unicode", "covers/{ASTRING}.put_unicode"
		local
			str: ASTRING; str_32: STRING_32; i: INTEGER
		do
	...
	test_replace_character
		note
			testing:	"covers/{ASTRING}.make_from_unicode",
						"covers/{ASTRING}.replace_character"
		local
			str: ASTRING; str_32: STRING_32; i, j: INTEGER
			uc: CHARACTER_32
		do
			str := Tao; str_32 := Tao.twin
	...
	test_replace_substring
		note
			testing:	"covers/{ASTRING}.make_from_unicode", "covers/{ASTRING}.replace_substring",
						"covers/{ASTRING}.substring_index"
		local
			str: ASTRING; str_32, replacement: STRING_32
			index: INTEGER
		do
	...
	test_replace_substring_all
		local
			target_32, old_target_32: STRING_32; target, old_target: ASTRING
	...
	test_sort
		local
			sorted: SORTED_TWO_WAY_LIST [ASTRING]; sorted_32: SORTED_TWO_WAY_LIST [STRING_32]
			word: ASTRING
		do
	...
	test_string_search
		...
		note
			testing:  "covers/{EL_STRING_SEARCHER}.substring_index_with_deltas",
			          ...
			          "covers/{ASTRING}.substring",
			          "covers/{ASTRING}.append_unicode"
		local
			substrings: EL_OCCURRENCE_SUBSTRINGS
			str, search_str, substring: ASTRING
			count: INTEGER
		do
	...
	test_unescape
		note
			testing:	"covers/{ASTRING}.make_from_unicode", "covers/{ASTRING}.unescape",
						"covers/{ASTRING}.put_unicode"
		local
			escaped: ASTRING
		do
			escaped := C_escaped
	...
	test_word_search (str_32: STRING_32)
		...
		do
			...
			across str.split (' ') as word loop
				...
				assert ("word found ASTRING", str.substring (pos, pos + word.item.count - 1) ~ word.item)
...
feature {NONE} -- Implementation
	...
	adjust (str: ASTRING; str_32: STRING_32; left: BOOLEAN)
		do
			if left then
	...
	new_text_files_list: EL_FILE_PATH_LIST
		...
		do
			...
			across 0 |..| Max_count as count loop
				Result.extend (Template #$ [count.item])
	...
	unicode_index_of (str_32: STRING_32; uc: CHARACTER_32)
		local
			str: ASTRING; part_a: ASTRING; part_32: STRING_32
		do
			str := str_32
	...
	unicode_split (a_str_32: STRING_32; a_separator: CHARACTER_32)
		local
			str: ASTRING; strings_a: LIST [ASTRING]; strings_32: LIST [STRING_32]
			ok: BOOLEAN
		do
			str := a_str_32
