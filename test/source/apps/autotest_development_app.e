note
	...
	date: "2016-06-23 19:42:20 GMT (Thursday 23rd June 2016)"
	revision: "8"
...
feature -- Basic operations
	...
	test_audio_commands
		local
			test_set: AUDIO_COMMAND_TEST_SET
		do
			create test_set
			test_set.test_mp3_audio
	...
	test_os_commands
		local
			test_set: OS_COMMAND_TEST_SET
		do
			create test_set
			test_set.test_user_list
			test_set.clean (False)
		end

	test_os_file_commands
		local
			test_set: FILE_COMMAND_TEST_SET
		do
			create test_set
			test_set.test_gnome_virtual_file_system
			test_set.clean (False)
		end
...
feature -- Tests
	...
	directory_creation (wav_path: EL_DIR_PATH)
		...
		do
			...
			if test_dir.is_createable then
				...
				log.put_string (" created")
				log.put_new_line
				File_system.copy_file (wav_path + "pop.wav", test_dir)
			else
				log.put_string_field ("Path", test_dir.to_string)
				log.put_string (" not creatable")
				log.put_new_line
	...
	file_and_directory_creation_with_latin1_chars (dir_path: EL_DIR_PATH)
		...
		do
			...
			File_system.move_file (file_path, dir_path.joined_file_steps (
				<< "Enrique Rodríguez", "No te Quiero Más.wav" >>)
			)
			log.exit
		end
	...
	Log_filter: ARRAY [like Type_logging_filter]
		...
		do
			Result := <<
				...
				[{EL_WAV_GENERATION_COMMAND_IMP}, All_routines],
