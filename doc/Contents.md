...
The descendants of [EL_BENCHMARK_COMPARISON](https://github.com/finnianr/Eiffel-Loop/library/testing/benchmark/el_benchmark_comparison.e) below were created to benchmark particular code characteristics. Any of the benchmarks can be run from a menu shell.

The class [DEVELOPER_COMPARISON](https://github.com/finnianr/Eiffel-Loop/benchmark/source/comparison/developer_comparison.e) is used to create throw-away benchmarks that  answer particular questions. After benchmarking new code, it is discarded but the results are documented in the section "Benchmark Journal" below.
...
Finding a [ZSTRING](https://github.com/finnianr/Eiffel-Loop/library/base/text/zstring/el_zstring.e) type at the end of list of 500 STRING_8 objects.
...
Method for [EL_INTEGER_MATH](https://github.com/finnianr/Eiffel-Loop/library/base/math/el_integer_math.e)
...
* Support for reading from objects conforming to [EL_READABLE](https://github.com/finnianr/Eiffel-Loop/library/base/kernel/support/el_readable.e) and writing to objects conforming to [EL_WRITABLE](https://github.com/finnianr/Eiffel-Loop/library/base/runtime/io/medium/el_writable.e)
...
To make an Eiffel class serializable with *Evolicity* you inherit from class [EVC_SERIALIZEABLE](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/serialization/evc_serializeable.e). Read the class notes for details on how to use. You can also access the substitution engine directly from the shared instance in class [EVC_SHARED_TEMPLATES](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/evc_shared_templates.e)
...
* Has a class [EVC_CACHEABLE_SERIALIZEABLE](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/serialization/evc_cacheable_serializeable.e) for caching the substituted output. Useful for generating pages on a web-server.

**VARIABLE CONTEXTS**

Variables which can be referenced in the template are specified by implementing the function `getter_function_table` in classes inheriting [EVC_EIFFEL_CONTEXT](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/context/evc_eiffel_context.e). It returns a table mapping variable names to agent functions.
...
A basic type is one of: [READABLE_STRING_GENERAL](https://www.eiffel.org/files/doc/static/trunk/libraries/base/readable_string_general_chart.html), [INTEGER_32_REF](https://www.eiffel.org/files/doc/static/trunk/libraries/base/integer_32_ref_chart.html), [NATURAL_32_REF](https://www.eiffel.org/files/doc/static/trunk/libraries/base/natural_32_ref_chart.html), [BOOLEAN_REF](https://www.eiffel.org/files/doc/static/trunk/libraries/base/boolean_ref_chart.html) or [REAL_64_REF](https://www.eiffel.org/files/doc/static/trunk/libraries/base/real_64_ref_chart.html)

**2.** Evolicity Context

A type conforming to [EVC_CONTEXT](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/context/evc_context.e) or [EVC_CONTEXT_IMP](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/context/evc_context_imp.e) or [EVC_SERIALIZEABLE](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/serialization/evc_serializeable.e). In the client template page, you use the standard feature call dot notation to select which object within the context you want to substitute into the template.

**3.** An Iterable List

An iterable list [ITERABLE](https://www.eiffel.org/files/doc/static/trunk/libraries/base/iterable_chart.html) where `G` recursively conforms to one of the types 1 to 3.

**STANDARD VARIABLES**

Contexts which inherit [EVC_SERIALIZEABLE](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/serialization/evc_serializeable.e) have a number of built-in standard variables. These are:

* **encoding_name:** the output encoding for the current template. For example: `UTF-8`
* **template_name:** the name of the current template. Internally this is of type [FILE_PATH](https://github.com/finnianr/Eiffel-Loop/library/base/runtime/file/naming/el_file_path.e).
...
When implementing the [EVC_EIFFEL_CONTEXT](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/context/evc_eiffel_context.e) table it is possible to assign functions with open arguments. You can then call the function from the template using this syntax:
...
2. A [REAL_64](https://www.eiffel.org/files/doc/static/trunk/libraries/base/real_64_chart.html) constant.
3. An [INTEGER_64](https://www.eiffel.org/files/doc/static/trunk/libraries/base/integer_64_chart.html) constant.
4. A quoted string.
...
type [SEQUENCE](https://www.eiffel.org/files/doc/static/trunk/libraries/base/sequence_chart.html).
...
**Iteration of [ITERABLE](https://www.eiffel.org/files/doc/static/trunk/libraries/base/iterable_chart.html) containers**

There are two loop syntax alternatives to iterate any object which conforms to the type [ITERABLE](https://www.eiffel.org/files/doc/static/trunk/libraries/base/iterable_chart.html) where `G` is an object that satisfies the condition [EVC_CONTEXT](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/context/evc_context.e).
...
````
The loop index can be referenced using the implicit variable: `$loop_index`. If in addition the container also conforms to [TABLE_ITERABLE](https://www.eiffel.org/files/doc/static/trunk/libraries/base/table_iterable_chart.html), then the table key value can be referenced by inserting an additional variable name, separated by a comma. This is similar to Python.
...
````
The object referenced by `<iterable-name>` must conform to type [ITERABLE](https://www.eiffel.org/files/doc/static/trunk/libraries/base/iterable_chart.html). Exactly like in Eiffel you reference the item values in the directive block using the syntax `$<variable-name>.item`, and the cursor index can be referenced as: `$<variable-name>.cursor_index`. If in addition the container also conforms to [TABLE_ITERABLE](https://www.eiffel.org/files/doc/static/trunk/libraries/base/table_iterable_chart.html), then the table key value can be referenced as `$<variable-name>.key`.
...
````
Here `<CLASS-NAME>` must be some type which conforms to type [EVC_SERIALIZEABLE](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/serialization/evc_serializeable.e) and therefore has a template. The variable in the second argument is some Eiffel data accessible as an Evolicity variable which is referenced by the nested template.
...
````
Here the first argument is a reference to an object that conforms to type [EVC_SERIALIZEABLE](https://github.com/finnianr/Eiffel-Loop/library/text/template/evolicity/serialization/evc_serializeable.e) and therefore has a template name which be referenced with the implicit variable name `template_name`.
...
````
Here the iterable container must conform to type [ITERABLE](https://www.eiffel.org/files/doc/static/trunk/libraries/base/iterable_chart.html). Note that even if the the nested text spans multiple lines, as it most likely will do, it will be indented to same indent level as the `#evaluate` directive.
...
[eXpat XML parser](http://expat.sourceforge.net/) of XML serializeable objects conforming to [EVOLICITY_SERIALIZEABLE_AS_XML](invalid-class-name).
...
[FILE_MANIFEST_APP](https://github.com/finnianr/Eiffel-Loop/tool/toolkit/source/apps/file_manifest_app.e) creates an XML file manifest of a target directory using either the default Evolicity template or an optional external Evolicity template. See class [EVOLICITY_SERIALIZEABLE](invalid-class-name)
